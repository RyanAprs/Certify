...
├── Events (CertificateIssued, MemberDecision, ZKVerified, …)
├── Replay protection (usedProofs mapping)
└── Dependencies: IGroth16Verifier interface

Groth16Verifier (auto-generated by snarkjs from range.circom)
└── verifyProof(a, b, c, [root, keyHash, threshold]) → Groth16 proof validation
```
...
**`verifyRangeProof` checks (in order):**

1. Certificate is `Active`.
2. `pubSignals[0]` (the circuit's Merkle root output) equals
   `cert.metadataCommitment` — binds the anonymous proof to this certificate.
3. `keccak256(abi.encode(a,b,c,pubSignals))` is not in `usedProofs` — replay guard.
4. `zkVerifier.verifyProof(...)` returns true.
5. Marks the proof used and emits `ZKVerified(certificateId, msg.sender, keyHash, threshold)`.
...
**File:** `zk/circuits/range.circom` (Phase 1: the range predicate)
...
| File | Size | Purpose |
|------|------|---------|
| `range.wasm` | 30KB | Circuit binary (browser-ready) |
| `range.zkey` | 3.2KB | Proving key (server-side) |
...
// Verify ZKP proof
verifyRangeProof(certificateId, a, b, c, pubSignals)

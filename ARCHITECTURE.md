...
│
├── lib/
│   ├── zkp.ts                      # Poseidon commitment + Groth16 proof
│   │   ├── commitmentFromMetadata()  # Poseidon(gpa, secret)
│   │   ├── generateGpaProof()
│   │   ├── verifyProof()
...
Groth16Verifier (auto-generated by snarkjs from certify.circom)
└── verifyProof(a, b, c, [commitment, minGpa]) → Groth16 proof validation
```
...
// Verifier — Groth16 selective GPA proof
function verifySelectiveProof(
    uint256 certificateId,
    uint[2] calldata a,
    uint[2][2] calldata b,
    uint[2] calldata c,
    uint[2] calldata pubSignals   // [commitment, minGpa]
) returns (bool)
```

**`verifySelectiveProof` checks (in order):**

1. Certificate is `Active`.
2. `pubSignals[0]` (the circuit's commitment output) equals
...
5. Marks the proof used and emits `ZKVerified(certificateId, msg.sender, minGpa)`.
...
template Certify() {
    // Private
    signal input gpa;      // scaled ×100 (e.g. 3.75 -> 375)
    signal input secret;   // random blinding factor

    // Public
    signal input minGpa;   // scaled ×100

    // Public output
    signal output commitment;

    // 1. Range-bound gpa so a field-wrapped value can't masquerade as valid.
    component gpaBits = Num2Bits(16);
    gpaBits.in <== gpa;

    // 2. REAL constraint: gpa >= minGpa (enforced, not implicit).
    component ge = GreaterEqThan(16);
    ge.in[0] <== gpa;
    ge.in[1] <== minGpa;
    ge.out === 1;

    // 3. Binding commitment via Poseidon (collision-resistant hash).
...
1. **GPA Validation — actually enforced**: `GreaterEqThan(16)` + `ge.out === 1`
   force `gpa >= minGpa`. (The *previous* circuit only assigned
   `diff <== actualGpa - minGpa` and never constrained it, so **any** GPA passed —
   that soundness hole is fixed here.)

2. **Commitment**: `Poseidon(gpa, secret)` — binding and collision-resistant.
   (The old `secret * metadataHash + actualGpa` was malleable and non-binding.)

**Public Signals Format** (snarkjs order: outputs, then public inputs):

```javascript
[
  commitment,  // pubSignals[0] - Poseidon(gpa, secret); must equal cert.metadataCommitment
  minGpa       // pubSignals[1] - Minimum GPA threshold that was proven
]
```
...
// 2. Commitment (at issue time, by the issuer)
const secret = generateSecret();                       // field element
const metadataCommitment = computeCommitment(385, secret); // Poseidon(gpa, secret)
// secret is stored inside the IPFS metadata so the holder can prove later.

// 3. Circuit inputs (at proof time)
const circuitInputs = {
  gpa: 385,        // private, scaled ×100
  secret,          // private blinding factor (from metadata)
  minGpa: 350      // public threshold, scaled ×100
};
...
publicSignals = [
  commitment,  // 0: Poseidon(gpa, secret)
...
function verifySelectiveProof(
    ...
    // 2. Commitment binds the anonymous proof to THIS certificate
    ...
    // 4. Verify Groth16 proof (which enforces gpa >= minGpa inside the circuit)
...
// Verify ZKP proof
verifySelectiveProof(certificateId, a, b, c, pubSignals)
...
- The circuit can reproduce `Poseidon(gpa, secret)` cheaply in-circuit, so the

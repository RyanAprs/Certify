...
## 4. Build the Zero-Knowledge Circuit (ZKP)

The circuit is compiled and its keys/verifier generated by one script.

**Prerequisite:** install `circom` v2 — https://docs.circom.io/getting-started/installation/
(`snarkjs` is invoked via `npx`, no global install needed).
...
### 4.2 Redeploy so the on-chain verifier is the real one

```bash
cd ../contracts && npm run deploy:local
```

✅ ZK proving artifacts + on-chain verifier ready.

> The trusted setup here uses dev entropy — fine for local/testnet, NOT mainnet
...
│   │   ├── verifier.sol              # Groth16Verifier (generated by zk/build.sh)
